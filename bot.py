...
# ✅ WEB APP
WEBAPP_URL = "https://tahirovdd-lang.github.io/bakery-chokoart/?v=2"
...
def tg_details(u: types.User | None, data: dict | None = None) -> str:
    data = data or {}
    ...
    if not username:
        raw_username = clean_str(tg_payload.get("username"))
        username = f"@{raw_username}" if raw_username else ""

    if not full_name:
        full_name = clean_str(tg_payload.get("first_name")) or clean_str(tg_payload.get("name"))

    if not user_id:
        user_id = clean_str(tg_payload.get("id"))
...
async def webapp_data(message: types.Message):
    ...
    admin_text = (
        ...
        f"\n📍 <b>Адрес:</b> {address}"
        f"\n📞 <b>Телефон:</b> {phone}"
        f"\n👤 <b>Telegram:</b> {tg_details(message.from_user, data)}"
    )
